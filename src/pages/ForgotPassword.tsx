    ...
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', backgroundColor: '#fff' }}>
      <div style={{ flex: '1', background: '#0d7c71', padding: '60px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', color: '#fff', position: 'relative', overflow: 'hidden' }}>
        ...
        <div>
          ...
          <div style={{ marginTop: '60px', maxWidth: '460px' }}>
            ...
            <h1 style={{ fontSize: '42px', fontWeight: '800', lineHeight: '52px', margin: '16px 0 20px 0', letterSpacing: '-1px' }}>Reset your staff account password.</h1>
            <p style={{ color: '#ccfbf1', fontSize: '17px', lineHeight: '26px', opacity: 0.95 }}>Provide your school ID and email to receive a secure reset link for your staff account.</p>
          </div>
        </div>
      ...
      <div style={{ width: '100%', maxWidth: '720px', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '60px 80px', boxSizing: 'border-box', backgroundColor: '#f8fafc' }}>
        <div style={{ width: '100%', maxWidth: '540px', margin: '0 auto' }}>
          ...
          <h2 style={{ margin: '0 0 8px 0', color: '#1a202c', fontSize: '32px', fontWeight: '800', letterSpacing: '-1px' }}>Reset Password</h2>
          <p style={{ margin: '0 0 28px 0', color: '#718096', fontSize: '15px', lineHeight: '22px' }}>Enter your email and school ID to receive a secure reset link.</p>

          {status === 'success' ? (
            <div style={{ backgroundColor: '#ecfdf5', color: '#065f46', padding: '14px 16px', borderRadius: '8px', border: '1px solid #bbf7d0', fontSize: '14px', marginBottom: '20px' }}>{message}</div>
          ) : (
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <input type="text" required value={schoolId} onChange={(e) => setSchoolId(e.target.value)} placeholder="e.g. 6574834" style={{ width: '100%', padding: '12px 14px', borderRadius: '8px', border: '1px solid #cbd5e0', boxSizing: 'border-box', fontSize: '15px', outline: 'none' }} />
              </div>
              ...
              <button type="submit" disabled={status === 'loading'} style={{ width: '100%', padding: '16px', background: '#0d7c71', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer', boxShadow: '0 4px 12px rgba(13, 124, 113, 0.12)', opacity: status === 'loading' ? 0.6 : 1 }}>{status === 'loading' ? 'Sending...' : 'Send Reset Link'}</button>
            </form>
          )}
